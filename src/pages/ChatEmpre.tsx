...
import { api } from "../lib/api"; // 👈 Usamos tu api existente
...
export function Chat() {
  const projectId = useMemo(
    () => getQueryParam("projectId") || "Proyecto Desconocido",
    []
  );
  ...
  const handleSendMessage = async () => {
    ...
    // 👇 Si es el chatbot, enviamos el mensaje al webhook de n8n
    if (projectId === "chatbot") {
      try {
        const response = await api.chatbot<{ output: string }>({
          message: newMessage,
        });
        ...
        const botMessage: Message = {
          id: Date.now() + 1,
          text: response.output || "El chatbot no respondió.",
          sender: "emprendedor",
          timestamp: new Date().toLocaleTimeString("es-AR", {
            hour: "2-digit",
            minute: "2-digit",
        ...
        const botError: Message = {
          id: Date.now() + 2,
          text: "Error al conectar con el chatbot de n8n 😢",
          sender: "emprendedor",
          timestamp: new Date().toLocaleTimeString("es-AR", {
            hour: "2-digit",
            minute: "2-digit",
      ...
      // Si no es el chatbot, usamos la simulación
      setTimeout(() => {
